...
list2env(purrr::map(set_names(parks_18, make_names(parks_18)), read_csv), envir = .GlobalEnv)
...
## liz wrote this code right below

# write.csv(agencies_bound, "N:/CommDev/Research/Research/Liz_Roten/regionalParks-SL-estimates/data/counts/2018_counts_all.csv")
...
step_II <- agencies_bound %>%
  ...
  mutate(Park_trail = ifelse(Park_trail == 'Minnesota River Greenway',
                             ...
                             Park_trail)) %>% # this line could possibly be removed
...
# write_csv(agency_comparison_percent, 'Results/Summer Visitor Use Estimates Aggregated by Agency.csv')
# write_csv(park_comparison_percent, 'Results/Summer Visitor Use Estimates Aggregated by Park.csv')
# write_csv(estimates_comparison_tidy, 'Results/Summer Visitor Use Estimate 2013 to 2018.csv')
```
...
```{r}
write_csv(annual_estimates_13_18, 'Results/Annual Use Estimates 2013 through 2018.csv')
```
...
boxplot_18 <- x2018 %>%
  mutate(Total_visitors = (ppv_multiplier*as.numeric(Vehicles) + as.numeric(Bikers) + as.numeric(Peds_skaters) + ppb_multiplier*as.numeric(Buses) + as.numeric(Horse_riders) + as.numeric(Boaters))) %>%
  ggplot(aes(Park_name, Total_visitors)) +
